...
This harness answers that question on a real production workload — [Sift](/work/sift)'s news-pipeline stages (categorization, summarization, structured extraction, grounded RAG) — across nine LLMs (five open-weight on a local DGX Spark via Ollama; four closed-weight via API). The deliverable is a routing-decision framework, not a benchmark leaderboard. The leaderboard is the worked example.
...
- `holdout.sha256` is committed to git *before any prompt iteration begins*.
- Prompt-iteration scripts only have read access to `data/dev/`; the runner requires an explicit `--include-held-out` flag.
- The final-scoring run commits results alongside the unchanged hash file. Any reviewer can verify (a) the hash hasn't changed since the pre-iteration commit, (b) the runner invocation logs include the held-out flag only on the final run.
