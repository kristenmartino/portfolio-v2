...
<Specs>
  ...
  <Spec label="Stack" value="Dash · Cloud Run + Memorystore Redis + GCS · XGBoost · Prophet · SARIMAX" />
...
Forecasting alone is insufficient — confidence is what enables decision-making. GridPulse runs three forecasting models (Prophet for additive seasonality, SARIMAX for stationary structure, XGBoost for non-linear weather-demand response) plus an inverse-MAPE weighted ensemble that down-weights underperforming models automatically. Backtests and confidence intervals are surfaced at the operating layer, not buried inside model views. When model spread widens, the platform surfaces the uncertainty rather than presenting a single value.
