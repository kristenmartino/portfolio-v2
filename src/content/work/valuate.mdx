...
For banks the workspace switches to a Gordon dividend-discount model: `P = D₀(1 + g) / (r − g)`, where the cost-of-equity and dividend-growth sliders replace WACC and terminal growth. For insurers it switches to a justified price-to-book multiple: `fair_value/share = book_value/share × (ROE − g) / (r − g)`, with reserves and the general-account investment portfolio surfaced alongside the standard balance-sheet metrics. For REITs it switches to an FFO-multiple Gordon model: `fair_value/share = FFO/share × (1 + g) / (r − g)`, where `FFO = net income + D&A` — the standard REIT cash-earnings adjustment, since GAAP depreciation overstates the economic depreciation of well-maintained real estate. For E&P companies it runs a 10-year reserve-life-capped FCFF with **no terminal value** — Gordon-growth-to-infinity is conceptually wrong for an asset whose proved reserves will run out, and the slider for revenue growth is relabeled "production growth/decline." Sensitivity is hidden for the bank / insurer / REIT flavors because rev-growth × op-margin don't enter their formulas; for E&P the heatmap stays on, since the FCFF math still uses both axes. The valuation flavor is a function of `period.industry` end-to-end — schema variants on the wire (where the line items differ), dispatch in `compute_projection`, slider relabeling on the frontend — rather than special cases grafted on top of the FCFF flow.
...
- **The original universe was 10 industrial / tech tickers; the architecture expanded to four additional industries.** Each was initially parked in the V2 column for the same reason most demos hide their scope ceiling: the FCFF schema doesn't fit a business whose primary product is interest spread (banks), one whose largest expense is claims/benefits incurred (insurers), one whose accounting depreciation systematically understates the cash it actually distributes (REITs), or one whose primary asset is depleting and won't justify a perpetual terminal value (E&P). The architecture expanded cleanly — discriminated unions on each statement, a per-industry concept map, an industry-aware composer, and a per-industry valuation flavor dispatched in `compute_projection` — rather than requiring a parallel codebase. The E&P case is the principled "dispatch-only" variant: the line items E&P companies report (revenue, op income, capex, D&A) are standard us-gaap, so there's no separate schema variant; what differs is the valuation math (10-year FCFF with no terminal value), which lives entirely in `dcf.py`.
- **Three of the original ten tickers needed Track B or derivation to compose.** XBRL tagging consistency turned out to be worse than the universe size suggests — even among hand-picked clean-reporting filers, ~30% have at least one required line item that's untagged or under a non-canonical concept. JPM, PRU, PLD, and EOG by contrast all extracted cleanly through Track A alone (post-CECL bank tags, life-insurer reserve tags, REIT real-estate tags, and the E&P-specific oil-and-gas-property and depletion tags are each well-standardized within their own taxonomy). The two-track-plus-derivation architecture earns its keep on industrials; the per-industry XBRL maps are the variant filers' clearest example of why having a structured fallback for non-canonical schemas was the right call from day one.
