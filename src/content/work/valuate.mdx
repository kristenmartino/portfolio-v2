...
- **The original universe was 10 industrial / tech tickers; banks, insurers, and REITs landed as the first three variants.** All three were initially parked in the V2 column for the same reason most demos hide their scope ceiling: the FCFF schema doesn't fit a business whose primary product is interest spread, one whose largest expense is claims/benefits incurred, or one whose accounting depreciation systematically understates the cash it actually distributes. The architecture turned out to expand cleanly — discriminated unions on each statement, a per-industry concept map, an industry-aware composer, and a per-industry valuation flavor (DDM for banks, justified P/B for insurers, FFO multiple for REITs) dispatched in `compute_projection` — rather than requiring a parallel codebase. Energy E&P remains Phase 4 work, with its stub already wired into the SIC classifier.
- **Three of the original ten tickers needed Track B or derivation to compose.** XBRL tagging consistency turned out to be worse than the universe size suggests — even among hand-picked clean-reporting filers, ~30% have at least one required line item that's untagged or under a non-canonical concept. JPM, PRU, and PLD by contrast all extracted cleanly through Track A alone (post-CECL bank tags, life-insurer reserve tags, and REIT real-estate tags are each well-standardized within their own taxonomy). The two-track-plus-derivation architecture earns its keep on industrials; the per-industry XBRL maps are the variant filers' clearest example of why having a structured fallback for non-canonical schemas was the right call from day one.
