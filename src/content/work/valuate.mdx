...
For banks the workspace switches to a Gordon dividend-discount model: `P = D₀(1 + g) / (r − g)`, where the cost-of-equity and dividend-growth sliders replace WACC and terminal growth. The same Monte Carlo loop runs over those two drivers; sensitivity is hidden because the rev-growth × op-margin axes don't enter DDM. The valuation flavor is a function of `period.industry` end-to-end — schema variants on the wire, dispatch in `compute_projection`, slider relabeling on the frontend — rather than a special case grafted on top of the FCFF flow.
...
- **The original universe was 10 industrial / tech tickers; banks landed as the first variant.** Banks were initially parked in the V2 column for the same reason most demos hide their scope ceiling: the FCFF schema doesn't fit a business whose primary product is the spread between deposit costs and loan yields. The architecture turned out to expand cleanly — discriminated unions on each statement, a per-industry concept map, an industry-aware composer, and a DDM dispatch in `compute_projection` — rather than requiring a parallel codebase. Insurers, REITs, and energy E&P remain Phase 2-4 work, with their stubs already wired into the SIC classifier.
