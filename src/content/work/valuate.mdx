...
The agent extracts each line item through one of two tracks, with confidence calibration and a deterministic derivation step for fields that neither track can fill.
...
The `Company` is composed at the end, after both tracks plus derivation have run. Required fields that even derivation can't reach raise an explicit error. Validation flags low-confidence items (<0.80) and balance-sheet identity violations (>50bps). Overrides are persisted as `LineItem` entries with `source=USER_OVERRIDE` and re-trigger validation on each write.
...
- **Three of ten tickers needed Track B or derivation to compose.** XBRL tagging consistency turned out to be worse than the universe size suggests — even among hand-picked clean-reporting filers, ~30% have at least one required line item that's untagged or under a non-canonical concept. The two-track-plus-derivation architecture earns its keep on this universe; expanding to messier filers (banks, insurers) doesn't make the structure less necessary, only more.
- **Persistence is the next real engineering step.** Overrides currently live in process memory — a Railway redeploy wipes them. Postgres + a small persistence layer is the most concrete remaining work; multi-period extraction (3-5 years of comparatives, which the schema already supports) lands behind that.
